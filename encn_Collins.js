...
class encn_Collins {
    ...
    async displayName() {
        ...
        if (locale.indexOf('CN') != -1) return '柯林斯';
        if (locale.indexOf('TW') != -1) return '柯林斯';
        ...
        function getCollins(doc) {
            ...
            for (const defNode of defNodes) {
                ...
                if (posNodes && posNodes.length) {
                    pos = `\n<span class='pos'>${T(posNodes[0])}</span>\n`;
                ...
                function getChnTran(ele) {
                    ...
                    if (result.startsWith('"（')) {
                        return `    <span class="chn_tran" style="margin-left:-7px;">${JSON.parse(result)}</span>\n`;
                    }
                    return `    <span class="chn_tran">${JSON.parse(result)}</span>\n`;
                }
                

                eng_tran = eng_tran ? `    <span class="eng_tran">${eng_tran}</span>\n` : '';
                definition += `${pos}<span class="tran">\n${eng_tran}${chn_tran}</span>\n`;
                ...
                if (exampleNodes && exampleNodes.length > 0 && maxexample > 0) {
                    ...
                    for (const [index, example] of exampleNodes.entries()) {
                        ...
                        definition += `    <li class='sent'>\n        <span class='eng_sent'>${eng_sent}</span>\n        <span class='chn_sent'>${chn_sent}</span>\n    </li>\n`;
                    }
                    definition += '</ul>\n';
                }
    ...
    span.cet  {margin:0 3px;padding:0 3px;font-weight: normal;font-size: 0.8em;color:white;background-color:#5cb85c;border-radius:5px;}
    span.pos  {text-transform:lowercase; font-size:0.9em; margin-right:0; padding:2px 4px; color:white; background-color:rgb(97, 179, 235); border-radius:5px;}
    span.tran {margin:0; padding:0;}
    span.eng_tran {margin-right:0; padding:0;}
    span.chn_tran {color:rgb(97, 179, 235);}
    ul.sents {font-size:0.8em; list-style:none inside; margin:12px 0; padding:3px 12px; background:rgb(220, 235, 245); border-radius:5px;}
    li.sent  {margin:0; padding:0;}
    span.eng_sent {margin-right:0;}
    span.chn_sent {color:rgb(97, 179, 235);}
        ...
        function getYoudao(doc) {
            ...
            for (const defNode of defNodes) {
                ...
                definition += `    <li class="ec">\n${pos}        <span class="ec_chn">${def}</span>\n    </li>\n`;
            }
            definition += '</ul>';
            let css = `<style>
    span.pos  {text-transform:lowercase; font-size:0.9em; margin-right:0; padding:2px 4px; color:white; background-color:rgb(97, 179, 235); border-radius:5px;}
    ul.ec, li.ec {margin:0; padding:0;}
    ul.ec {list-style:none inside;}
</style>\n`;
            notes.push({
                css,
                expression,
