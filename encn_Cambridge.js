...
class encn_Cambridge {
    ...
    async displayName() {
        ...
        if (locale.indexOf('CN') != -1) return '剑桥(简体)';
        if (locale.indexOf('TW') != -1) return '劍橋(簡體)';
                    ...
                    for (const defblock of defblocks) {
                        ...
                        if (examps.length > 0 && this.maxexample > 0) {
                            ...
                            for (const [index, examp] of examps.entries()) {
                                ...
                                definition += `    <li class='sent'>\n        <span class='eng_sent'>${eng_examp.replace(RegExp(expression, 'gi'),`<b>${expression}</b>`)}</span>\n        <span class='chn_sent'>${chn_examp}</span>\n    </li>\n`;
                            }
                            definition += '</ul>\n';
                        }
